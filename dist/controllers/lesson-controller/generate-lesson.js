...
exports.generateCourseWithTTS = generateCourseWithTTS;
const generative_ai_1 = require("@google/generative-ai");
const dotenv_1 = __importDefault(require("dotenv"));
const lesson_1 = require("../../prompt/lesson");
const getVoiceId_1 = __importDefault(require("../../utils/getVoiceId"));
const prisma_1 = require("../../utils/prisma");
dotenv_1.default.config();
const GOOGLE_KEY = process.env.GOOGLE_API_KEY;
const MURF_API_KEY = process.env.MURF_API_KEY;
// Generate TTS audio using Murf API
async function generateTTS(phrase, voiceID) {
    try {
 ...
 */
async function injectTTS(lessons, voiceID) {
    const ttsTasks = [];
...
async function generateCourseWithTTS(data) {
    try {
        if (!GOOGLE_KEY) {
            return {
                status: "401",
        ...
        const genAI = new generative_ai_1.GoogleGenerativeAI(GOOGLE_KEY);
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
        const systemPrompt = (0, lesson_1.lessonPrompt)(data);
        const result = await model.generateContent(systemPrompt);
        let raw = result.response.text();
        raw = raw.replace(/```json|```|\n/g, "").trim();
        const parsed = JSON.parse(raw);
        const voiceID = (0, getVoiceId_1.default)(data.native_lang);
